...
Implements the same fetch() -> list[Signal] interface as every other module in signals/
(src/signals/base.py) -- nothing downstream needs to know this source is Selenium-based
rather than an HTTP API call. Domain classification reuses
engagement.content_analyzer.analyze_post()'s domain_guess rather than duplicating keyword
matching here; posts that guess as 'general' (no clear tech/history/arts signal) are dropped
since Signal.domain is only ever one of those three elsewhere in the codebase.

Requires an authenticated session -- see utils/browser.py's ensure_logged_in(). Each fetch()
call opens and closes its own WebDriver, matching how the other signal sources make
self-contained calls rather than holding state open across calls.
"""
...
def fetch(max_results: int = 10) -> list[Signal]:
    """Scrape the current home timeline into Signals, newest/topmost first."""

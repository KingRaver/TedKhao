...
def test_tied_top_scores_not_order_biased() -> None:
    ...
    random.seed(7)
    phase1, _, signal1, _ = select_phase_register_and_signal(pool_arxiv_first)
    random.seed(7)
    phase2, _, signal2, _ = select_phase_register_and_signal(pool_reversed)
    ...
    for seed in range(40):
        random.seed(seed)
        _, _, signal, _ = select_phase_register_and_signal(pool_arxiv_first)
...
def test_history_only_pool_top_rank_is_anniversary_not_breakthrough() -> None:
    ...
    random.seed(1)
    phase, _, signal, _ = select_phase_register_and_signal(signals)
...
def test_unrelated_cross_domain_high_novelty_is_not_convergence() -> None:
    ...
    random.seed(1)
    phase, _, signal, _ = select_phase_register_and_signal([tech, arts])
...
def test_related_cross_domain_high_novelty_is_convergence() -> None:
    ...
    random.seed(1)
    phase, _, signal, partner = select_phase_register_and_signal([tech, arts])
    assert phase == Phase.CONVERGENCE, (
        f"two different-domain high-novelty signals sharing 'Rembrandt' selected {phase}, "
        "not Convergence"
    )
    assert signal.source in {"arxiv", "met_museum"}
    # RC-108: Convergence's whole premise is two signals rhyming, so both must actually reach
    # the caller -- a prior version of this selection silently dropped the second one.
    assert partner is not None, "Convergence must surface a convergence_partner signal"
    assert partner is not signal, "convergence_partner must be the other signal in the pair"
    assert {signal.source, partner.source} == {"arxiv", "met_museum"}
    print("  related cross-domain high-novelty items (shared vocabulary) -> Convergence, both "
          "signals surfaced: OK")
...
def test_empty_pool_is_quiet_with_no_signal() -> None:
    phase, _, signal, _ = select_phase_register_and_signal([])
    assert phase == Phase.QUIET
